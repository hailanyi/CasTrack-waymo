...
class Tracker3D:
    ...
    def tracking(self,bbs_3D = None,
        ...
        if self.current_bbs is None:
            return np.zeros(shape=(0,7)),np.zeros(shape=(0)),np.zeros(shape=(0))
        else:
            if len(self.current_bbs) == 0:
                ...
                bbs,ids, scores = self.trajectories_update_init(ids)

                return bbs,ids,scores
    ...
    def trajectories_update_init(self,ids):
        ...
        if len(valid_bbs)==0:
            ...
            return np.array(valid_bbs),np.array(valid_ids),np.array(valid_scores)
