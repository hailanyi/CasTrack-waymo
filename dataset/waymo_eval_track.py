...
from waymo_open_dataset.metrics.python import detection_metrics,tracking_metrics
...
class OpenPCDetWaymoDetectionMetricsEstimator(tf.test.TestCase):
    ...
    def generate_waymo_type_results(self, infos, class_names, is_gt=False, fake_gt_infos=True):
        ...
        frame_id, seq_id, boxes3d, obj_type, score, overlap_nlz, ids, difficulty, speed = [], [], [], [], [], [],[],[],[]
        ...
        for frame_index, info in enumerate(infos):
            if is_gt:
                ...
                this_ids_str = info['obj_ids'][box_mask]

                this_ids_int = []

                for i_str in this_ids_str:
                    if i_str in str2int_dict:
                        this_ids_int.append(str2int_dict[i_str])
                    else:
                        this_ids_int.append(iter)
                        str2int_dict[i_str]=iter
                        iter+=1

                ids.append(this_ids_int)
                speed.append(np.concatenate([info['speed_x'][box_mask].reshape(-1,1),info['speed_y'][box_mask].reshape(-1,1)],-1))
                ...
                if 'obj_ids' in info:
                    ids.append(info['obj_ids'])
                else:
                    ids.append(np.arange(num_boxes))

            obj_type += [self.WAYMO_CLASSES.index(name) for i, name in enumerate(box_name)]

            frame_id.append(np.array([info['frame_id']] * num_boxes))
            seq_id.append(np.array([info['seq_id']] * num_boxes))
            overlap_nlz.append(np.zeros(num_boxes))  # set zero currently

        frame_id = np.concatenate(frame_id).reshape(-1)
        seq_id = np.concatenate(seq_id).reshape(-1)
        ...
        ids = np.concatenate(ids).reshape(-1)
        if len(speed)!=0:
            speed = np.concatenate(speed)

        boxes3d[:, -1] = limit_period(boxes3d[:, -1], offset=0.5, period=np.pi * 2)

        return frame_id,seq_id, boxes3d, obj_type, score, overlap_nlz, difficulty,ids,speed
...
#      }
    ...
    def waymo_evaluation(self, prediction_infos, gt_infos, class_name, distance_thresh=100, fake_gt_infos=True):
        ...
        pd_frameid, pd_seq_id, pd_boxes3d, pd_type, pd_score, pd_overlap_nlz, _ ,pd_ids,_= self.generate_waymo_type_results(
            prediction_infos, class_name, is_gt=False
        )
        gt_frameid, gt_seq_id, gt_boxes3d, gt_type, gt_score, gt_overlap_nlz, gt_difficulty,gt_ids, gt_speed = self.generate_waymo_type_results(
        ...
        with self.test_session(graph=graph) as sess:
            ...
            self._EvalUpdateOps(sess, graph, metrics,pd_boxes3d,
                                pd_type,pd_score,
                                pd_frameid,pd_seq_id,
                                pd_ids,gt_boxes3d,
                                gt_type,gt_frameid,
                                gt_seq_id,gt_ids,
                                gt_speed,
...
def main():
    ...
    parser.add_argument('--pred_infos', type=str, default='/home/projects/wh/OpenProjects/OpenPCDet/output/multi_frame/waymo_models/voxel_rcnn_0.075/default/eval/epoch_25/val/default/result_merged.pkl', help='pickle file')
    parser.add_argument('--gt_infos', type=str, default='data/waymo/infos/waymo_infos_val.pkl', help='pickle file')
    parser.add_argument('--class_names', type=str, nargs='+', default=['Vehicle', 'Pedestrian', 'Cyclist'], help='')
    parser.add_argument('--sampled_interval', type=int, default=10, help='sampled interval for GT sequences')
    ...
    for idx in range(0, len(gt_infos), args.sampled_interval):
        ...
        sample_idx = gt_infos[idx]['point_cloud']['sample_idx']
        seq_idx = gt_infos[idx]['point_cloud']['lidar_sequence']
        cur_info['frame_id'] = sample_idx
        cur_info['seq_id'] = seq_idx
        gt_infos_dst.append(cur_info)

    ap_dict = eval.waymo_evaluation(
        gt_infos_dst, gt_infos_dst, class_name=args.class_names, distance_thresh=1000, fake_gt_infos=False
    )
